...
with zipfile.ZipFile(public/'course-materials.zip','w',zipfile.ZIP_DEFLATED) as z:
 for name in ['index.html','ai-office-startup.html','ai-office-startup-slides.html']:z.write(public/name,name)
 for folder in ['assets/ai-office-startup','materials/ai-office-startup']:
  for p in (public/folder).rglob('*'):
   if p.is_file():z.write(p,p.relative_to(public))
